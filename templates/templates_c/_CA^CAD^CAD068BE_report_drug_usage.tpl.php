<?php /* Smarty version 2.6.19, created on 2017-08-08 01:50:37
         compiled from report_drug_usage.tpl */ ?>
<!DOCTYPE html>
<html>
...
<?php echo '
 ...
 <?php echo '\'
    ...
    function get_result(ladda_btn){       
        ...
        $.get( "report.php", {act :"get_drug_usage_report" , start_date : $(\'#start_date\').datepicker("getDate").toDBString() , end_date : $(\'#end_date\').datepicker("getDate").toDBString()}).done(function( data ) {
            $(\'#barChart\').attr("class" , " animated fadeInDown");
            //sk-loading  start
            $(\'#report_content\').children(\'.ibox-content\').toggleClass(\'sk-loading\');
            //parse json array
			var result = JSON.parse(data);
			$(\'#report_detail\').html(result.table);	
            var dt =  result.chart;   		
            //set bar data
            var barData = {
                labels: dt[0],
