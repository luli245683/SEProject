...
$_SESSION["active_page"]='Candidate_assembly_article';
...
if($_GET["act"]=='start_Article'){
	...
	$article_list = get_Candidate_Article(null);//取得{柯文哲}文章資料庫
...
if($_GET["act"]=='article_search'){
	...
	$article_list =article_search($Context_val);
	
	$article_id_list = get_article_id($article_list);
	for ($i=0; $i < count($article_id_list); $i++) { 
		$article_list[$i]['reply'] = get_reply_Article($article_id_list[$i]);
